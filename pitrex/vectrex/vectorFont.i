...
void v_Draw_VLp(int8_t *list)
{
    do
    {
        ...
        WAIT_T1_END();
        if (((int8_t) ( *list) )>=0)
                    DELAY_CYCLES(5); // last vector should wait longer
        SWITCH_BEAM_OFF();
    }
    while (((int8_t) ( *list) )<=0);
}
...
/*
			  ...
			  cpb->y0 = startY;
			  cpb->x0 = startX;
			  cpb->y1 = (*(list+1))*SCALEFONT+startY;
			  cpb->x1 = (*(list+2))*SCALEFONT+startX;

			  startY = cpb->y1;
			  startX = cpb->x1;
			  cpb->timingForced = currentScale;
			  cpb->sms = MAX_USED_STRENGTH ;
			  cpb->force |= (commonHints| PL_BASE_FORCE_USE_FIX_SIZE);
