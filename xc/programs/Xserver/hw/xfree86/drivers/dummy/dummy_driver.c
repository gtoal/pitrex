 ...
 *
 * This has been roughly hacked to display with the PiTrex via XAA
 */
...
dummySetup(ModuleDescPtr module, pointer opts, int *errmaj, int *errmin)
{
	...
	xf86LoaderModRefSymLists(module, fbSymbols, xaaSymbols, NULL);
	/*
...
# define RETURN \
    ...
    /* PiTrex vector scaling options (do it this way instead of basing on resultion
     * so that the display is manually adjusted for best fit
     */
    dPtr->xscale = xf86SetRealOption(pScrn->options, "Xscale", 1);
    if (dPtr->xscale != 1) {
	xf86Msg(X_CONFIG, "PITREX XAA: X scale = %f\n", dPtr->xscale);
    }
    dPtr->yscale = xf86SetRealOption(pScrn->options, "Yscale", 1);
    if (dPtr->yscale != 1) {
	xf86Msg(X_CONFIG, "PITREX XAA: Y scale = %f\n", dPtr->yscale);
    }
    dPtr->xoffset = xf86SetIntOption(pScrn->options, "Xoffset", 0);
    if (dPtr->xoffset != 0) {
	xf86Msg(X_CONFIG, "PITREX XAA: X offset = %d\n", dPtr->xoffset);
    }
    dPtr->yoffset = xf86SetIntOption(pScrn->options, "Yoffset", 0);
    if (dPtr->yoffset != 0) {
                ...
                const int argc, const char **argv)
{
    ...
    /* Enable XAA (PiTrex line drawing) */
    pitrexXAAinit(pScreen);
