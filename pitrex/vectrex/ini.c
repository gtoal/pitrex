...
// system headers that work in both environments:
#include <stdio.h>
#include <string.h>
#include <error.h>
#include <errno.h>
...
#else
// imitate baremetal calls:
#include <unistd.h>
#define f_getcwd(buf,size) getcwd(buf,size)
static long __filelength(FILE *f) {
  long curpos = ftell(f), flen;
  fseek(f, 0L, SEEK_END);
  flen = ftell(f);
  fseek(f, curpos, SEEK_SET);
  return flen;
}
#define ff_getErrorText(no) strerror(no)
#endif
