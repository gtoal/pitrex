...
byte MEMRD(unsigned addr, int PC, unsigned long cyc)
{
  ...
  switch(tag & 0x3f) 
	...
	{
	case 0: /* left coin */
	  result = !(check_switch_decr (& cslot_left) << 7);   // These next 3 results give a GCC warning:
                                                               // ‘<<’ in boolean context, did you mean ‘<’ ?
	                                                       // The problem is that !(expr) implies that
	                                                       // expr is 0 or non-0 - in which case, what
	                                                       // difference does << 7 make?  If it makes
	                                                       // no difference, why is it there? It implies
	                                                       // there's something amiss here...
	                                                       // Could it be a ~ needed rather than ! for
	                                                       // example?  Or maybe (!(expr))<<7 was meant?
	  break;
	case 1: /* center coin */
	  result = !(check_switch_decr (& cslot_util) << 7);
	  break;
